...
const deletePatient = async (req, res) => {
  try {
    ...
    const patientId = parseInt(id, 10);      // Define patientId
    ...
    // Delete related Grad-CAM scan records first
    await prisma.gradCamScan.deleteMany({
      where: { patientId: patientId },
    });
    
    // Delete related MRI scan records first
    await prisma.mriScan.deleteMany({
      where: { patientId: patientId },
    });
